...
export default {
  ...
  methods: {
    saveButton() {
      var editedContact = {
        ...
        firstName: this.contactData.firstName.toLowerCase(),
        lastName: this.contactData.lastName.toLowerCase(),
        mobileNo: this.contactData.mobileNo,
        email: this.contactData.email.toLowerCase(),
        facebook: this.contactData.facebook.toLowerCase(),
      ...
      if (this.contactData.contactId && this.contactData.firstName && this.contactData.lastName && this.contactData.mobileNo) {
        axios
          .post(
            "http://localhost:3000/contact/" + this.$route.params.contactId,
            editedContact
          )
          .then(response => {
  ...
  mounted() {
    axios
      .get("http://localhost:3000/contact/" + this.$route.params.contactId)
      .then(response => {
